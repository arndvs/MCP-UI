/**
 * MCP Tools Solution - Raw HTML UI Components
 * 
 * This file demonstrates how to transform plain text responses into rich visual interfaces
 * using MCP UI. The key innovation here is the 'view_tag' tool which returns HTML content
 * that gets rendered as actual visual components instead of plain text.
 * 
 * Key concepts:
 * - createUIResource: Packages HTML content into a format MCP clients can understand
 * - URI pattern: ui://view-tag/{id} creates unique identifiers for UI resources
 * - rawHtml type: Specifies that the content is raw HTML that should be rendered
 * - Visual transformation: Users get immediate, scannable visual cards instead of text walls
 */
...
// Import createUIResource to enable rich HTML UI components
// This allows us to transform plain text responses into visual interfaces
...
export async function initializeTools(agent: EpicMeMCP) {
	...
	agent.server.registerTool(
		...
		async ({ id }) => {
			...
			// Create HTML content based on whether the tag exists
			// This transforms plain text responses into rich visual interfaces
			const html = tag
				? `
				<div>
					<h1>${tag.name}</h1>
			...
			// Return a UI resource instead of plain text
			// This allows MCP clients to render the HTML as a visual component
			// The URI follows the pattern: ui://view-tag/{id} for unique identification
			return {
				content: [
					createUIResource({
						uri: `ui://view-tag/${id}`,
	...
	// Note: This tool differs from 'view_tag' in that it returns structured data
	// instead of visual HTML. 'view_tag' is for visual presentation, while 'get_tag'
	// is for programmatic access to tag data.
	agent.server.registerTool(
		'get_tag',
		{
			title: 'Get Tag',
