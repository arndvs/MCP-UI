...
// DYNAMIC SIZING LESSON: This component demonstrates how to communicate
// its size requirements to a parent iframe/agent so the container can
// adjust accordingly. This prevents unnecessary scrolling and ensures
// optimal display of content.
//
// KEY LEARNING POINTS:
// 1. useRef() provides direct access to DOM elements for measurement
// 2. clientHeight/clientWidth give us the visible dimensions (not scroll dimensions)
// 3. postMessage() enables communication between iframe and parent window
// 4. Dynamic sizing improves UX by fitting content perfectly without wasted space
//
// WORKFLOW:
// 1. Component mounts and renders content
// 2. useEffect measures the actual rendered size
// 3. Sends 'ui-size-change' message to parent with dimensions
// 4. Parent (agent) resizes iframe to match reported dimensions
// 5. Result: Perfect fit, no unnecessary scrolling or empty space
...
export default function JournalViewer({ loaderData }: Route.ComponentProps) {
	...
	// This allows us to measure its actual rendered dimensions
	const rootRef = useRef<HTMLDivElement>(null)

	useEffect(() => {
		// First, notify parent that iframe is ready
		window.parent.postMessage({ type: 'ui-lifecycle-iframe-ready' }, '*')

		// 🎯 CORE DYNAMIC SIZING LOGIC:
		const root = rootRef.current
		if (!root) return // Safety check - ref might not be attached yet

		// Use clientHeight/clientWidth, NOT scrollHeight/scrollWidth
		// clientHeight = visible content height (what user sees)
		// scrollHeight = total content height (including hidden scrollable content)
		// 
		// Using scrollHeight will make the iframe too tall
		// because it includes content that's hidden by scrolling
		const height = root.clientHeight
		const width = root.clientWidth

		// Tell parent window our ideal dimensions
		// This allows the agent/iframe container to resize accordingly
	...
	return (
		<div
			ref={rootRef} // This connects our ref to the DOM element
			className="bg-background max-h-[800px] overflow-y-auto p-4"
			// max-h-[800px] ensures we don't grow infinitely large
			// overflow-y-auto provides scrolling when content exceeds max height
		>
