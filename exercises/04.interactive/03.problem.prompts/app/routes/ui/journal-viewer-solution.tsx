...
import { sendMcpMessage } from '#app/utils/mcp-solution.ts'
...
function SummarizeEntryButtonImpl({
	...
	const handleSummarize = () => {
		startTransition(async () => {
			try {
				// Send a prompt to the AI agent requesting a summary
				// The agent will use the EpicMe get_entry tool to retrieve the entry data
				// and then provide a concise summary based on that information
				//
				// We ask the agent to call the tool rather than including all entry data
				// in the prompt because: 1) Users see the prompt, so keeping it concise is better UX,
				// and 2) This demonstrates the agent's ability to use tools dynamically
